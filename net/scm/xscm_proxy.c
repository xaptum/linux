...
struct scm_proxy_inst {
	void *usb_context;
	struct scm_usb_descriptor *usb_intf;
	atomic_t scm_msg_id;
	...
	void *context)
{
	...
	proxy_inst->usb_intf->scm_cmd((char*)packet,
...
int scm_proxy_open_socket(int local_id, void *context)
{
	...
	proxy_inst->usb_intf->scm_cmd((char*)packet, sizeof(struct scm_packet),
		proxy_inst->usb_context);
...
void scm_proxy_close_socket(int local_id, void *context)
{
	...
	proxy_inst->usb_intf->scm_cmd((char*)packet, sizeof(struct scm_packet_hdr),
		proxy_inst->usb_context);
...
int scm_proxy_write_socket(int sock_id, void *msg, int len, void *context)
{
	...
	proxy_inst->usb_intf->scm_transfer(&packet, (char*)msg, len,
		proxy_inst->usb_context);
	return len;
}
